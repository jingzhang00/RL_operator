...
class ManipulatorEnv(gym.Env):
    ...
    def step(self, action, task, base_block):
        env_action = self.action_multiplier * action
        ...
        assert len(env_action) == valid_len, "len of action is %d, must be %d for this environment" % \
                                                  (len(env_action), valid_len)

        fixed_action[0][valid_t_dofs] = env_action[:len(valid_t_dofs)]
        fixed_action[1][valid_r_dofs] = env_action[len(valid_t_dofs):(len(valid_t_dofs) + len(valid_r_dofs))]
        if self.grip_in_action:
            if self.env.gripper_control_method == 'bool_p':
                if env_action[-1] < 0:  # no support for position based grasping yet
                    fixed_action[2] = False
                else:
                    fixed_action[2] = True
            elif self.env.gripper_control_method == 'dp':
                fixed_action[2] = env_action[-1] / self.action_multiplier * self.grip_multiplier
            else:
                raise NotImplementedError("Not implemented for gripper control method %s" %
                                          self.env.gripper_control_method)
        env_action = fixed_action

        # now action is a 2 or 3 tuple, depending on if gripper in action, with first 2 elements trans and rot vel
        limit_reached = False

        for i in range(self.n_substeps):
            obs_dict, reward, done, _ = self.env.step(env_action)
        ...
        reward, task_state, main_task_state, effect, base_block = self._calculate_reward_and_done(action, task, base_block)
        ...
        return_obs['task_state'] = task_state
        return_obs['main_task_state'] = main_task_state
        return_obs['effect'] = effect
        return_obs['done_failure'] = done_failure
        return_obs['curr_task'] = task
        return_obs['main_task'] = self.main_task
        return_obs['base_block'] = base_block

        und_loc = self.main_task.rfind('_')
        suffix = [int(suf_char) for suf_char in list(self.main_task[und_loc + 1:])]
        block_num = len(suffix)
        return_obs['block_num'] = block_num

        self.ep_timesteps += 1
        done = False
        if self.ep_timesteps >= self._max_episode_steps:
            done = True
        if return_obs['main_task_state']:
            done = True
