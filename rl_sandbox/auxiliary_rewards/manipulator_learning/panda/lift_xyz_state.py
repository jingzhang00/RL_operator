...
def dense_reward(obj_1_pos, obj_2_pos, tanh_multiplier=10.0):
    dist = norm(obj_1_pos - obj_2_pos)
    return 1.5 * (1 - np.tanh(tanh_multiplier * dist))


def away(obj_1_pos, obj_2_pos, multiplier=50 * np.pi):
    dist = norm(obj_1_pos - obj_2_pos)
    if dist > 0.02:
        return 1
    else:
        return np.sin(multiplier * dist - 0.5 * np.pi)
