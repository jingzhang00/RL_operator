...
class SACXAgent(HierarchicalRLAgent):
    ...
    def compute_action(self, obs, hidden_state, curr_env, action):
        env_info = getattr(curr_env, "_return_obs")
        task_state = env_info.get("task_state", False)
        if (self._curr_timestep == 0) or (self._curr_timestep == 1) or (task_state == True) or (self.task_timestep % self._scheduler_period == 0):
            self.task_timestep = 0
            ...
            self.curr_high_level_act, self.task, self.base_block, self.curr_high_level_value, self.curr_high_level_h_state, \
                self.curr_high_level_log_prob, self.curr_high_level_entropy, self.curr_high_level_mean, self.curr_high_level_variance = \
                    self.high_level_model.compute_action(self.curr_high_level_obs, self._curr_timestep, curr_env, action, torch.tensor(self.curr_high_level_h_state))
        ...
        return action[self.curr_high_level_act], hidden_state, act_info, task, base_block


    def deterministic_action(self, obs, hidden_state, env):
        env_info = getattr(env, "_return_obs")
        effect = env_info.get("effect", False)
        if isinstance(effect, dict):
            task_state = any(effect.values())
        else:
            task_state = effect
        if self._curr_timestep % self._scheduler_period == 0 or (task_state is True):
            if self.curr_high_level_obs is not None:
                self.curr_high_level_obs.append(self.curr_high_level_act.item())
            else:
                self.curr_high_level_obs = []
            ...
            self.curr_high_level_act, self.task, self.base_block, self.curr_high_level_value, self.curr_high_level_h_state, \
                self.curr_high_level_log_prob, self.curr_high_level_entropy = \
                    self.high_level_model.deterministic_action(self.curr_high_level_obs, torch.tensor(self.curr_high_level_h_state), env)
