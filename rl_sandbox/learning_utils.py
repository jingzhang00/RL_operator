...
def train(agent,
    ...
    main_task = experiment_settings["env_setting"]["kwargs"]["main_task"]
    ...
    try:
        ...
        for timestep_i in range(cum_episode_lengths[-1], max_total_steps):
            ...
            action, next_h_state, act_info, curr_task, base_block = agent.compute_action(
                obs=curr_obs, hidden_state=curr_h_state, curr_env=train_env, action=action)
            ...
            next_obs, reward, done, env_info = train_env.step(env_action, curr_task, base_block)
            next_obs = buffer_preprocess(next_obs)
            e_info = env_info["infos"][-1]
            ...
            if evaluation_frequency > 0 and curr_timestep % evaluation_frequency == 0:
                if experiment_settings.get(c.EVALUATION_IN_PARALLEL, False):
                    ...
                    epo_per_epi_length = epoch_summary._summary["interaction_info/episode_lengths"]["content"]
                    max_num = epo_per_epi_length.count(360)
                    rate = (len(epo_per_epi_length) - max_num) / len(epo_per_epi_length)
                    summary_writer.add_scalar(f"{c.EVALUATION_INFO}/epoch_success_rate", rate, timestep_i)
                    epoch_summary.log(f"{c.EVALUATION_INFO}/{c.AVERAGE_RETURNS}", multitask_returns, axis=(0, 2))
            ...
            if curr_timestep % print_interval == 0:
                ...
                max_num = epo_per_epi_length.count(360)
                rate = (len(epo_per_epi_length) - max_num) / len(epo_per_epi_length)
                epoch_summary.log(f"{c.INTERACTION_INFO}/epoch_success_rate", rate)
    ...
    for e in range(num_episodes):
        ...
        while not done:
            ...
            if stochastic_policy:
                ...
                action, h_state, act_info, task, base_block = agent.deterministic_action(
                    obs=curr_obs, hidden_state=h_state, env=env)

            if clip_action:
                action = np.clip(action, a_min=min_action, a_max=max_action)

            next_obs, reward, done, env_info = env.step(action, task, base_block)
            next_obs = buffer_preprocess(next_obs)
                ...
                if success_ends_ep:
                    ...
                    if suc:
                        ...
                        eval_returns[-1] += np.atleast_1d(
                            ...
                            next_observation=next_obs, info=env_info, eval_flag=eval_flag)) * (env.unwrapped._max_episode_steps - (ts + 1))
